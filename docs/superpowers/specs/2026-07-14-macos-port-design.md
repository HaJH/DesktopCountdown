...
> 상태: **구현 완료 (2026-07-14).** 1~8단계 전부. 실기에서 동작 확인.
> 이 문서는 설계 원본이고, 구현 중 틀린 것으로 드러난 부분은 그 자리에 표시해 고쳤다.
> 실기 검증 결과는 [`../plans/macos-spike-result.md`](../plans/macos-spike-result.md).
>
> **남은 미확인 항목은 §8-2(Sonoma+ 클릭 시 데스크톱 표시)와 §8-4의 폰트 weight 극단값뿐이다.**
...
> **⚠ 구현 중 발견 (5단계): `CGDisplayCreateUUIDFromDisplayID`는 CoreGraphics에 없다.**
> 이름과 달리 **ColorSync.framework**가 export한다 (`ColorSyncDevice.h`). objc2-core-graphics에
> 바인딩이 없으므로 `#[link(name = "ColorSync", kind = "framework")]`로 직접 extern 선언한다.
>
> **⚠ 좌표계.** 공유 `layout::place`는 **y-down**이다 (`TopLeft`가 `monitor.y`, offset +y가 아래).
> Win32가 그렇게 주기 때문. AppKit은 y-up(주 화면 좌하단 원점)이므로 `MonitorInfo.rect`는
> **y-down으로 변환해서 publish**한다 (주 화면 좌상단 원점 — Windows와 같은 모양).
> 창을 놓을 때 `to_appkit`으로 되돌린다. 이 변환은 자기 자신의 역함수여야 하고, 테스트가 그걸 고정한다.
>
> **단위는 point다** (Windows는 물리 픽셀). `scale`이 Retina 배율을 따로 나르고 렌더러가 비트맵
> 크기에만 곱한다. 그래서 같은 `size_px` 값이 두 OS에서 비슷한 크기로 보이고, Retina에서 뭉개지지 않는다.
...
1. ~~**Stage Manager** 켠 상태에서 데스크톱 레벨 창의 가시성.~~
   **해소 (1단계).** 켜도 그대로 보이고, 창 막대에도 안 나타난다.
...
3. ~~**메뉴바 상단 1~2px 틈** 발생 여부 (§5.1)~~
   **해소 (1단계).** macOS 26에서 발생하지 않는다. `constrainFrameRect` 오버라이드 불필요 →
   `define_class!` 사용처 0. (구형 OS에서 재발하면 그때 넣는다.)
4. **DWrite weight 100–900 → CT weight −1.0~1.0 매핑 앵커값** (비공식 수치).
   **부분 해소 (3단계).** 구현했고 단조성·범위·앵커 적중을 테스트로 고정했다.
   다만 **앵커 수치 자체가 여전히 비공식**이다 — 실제 렌더된 굵기가 Windows와 비슷해 보이는지는
   사람 눈으로만 확인할 수 있다. 4단계에서 화면에 띄웠고 regular(400)은 이상 없어 보였으나,
   **100/900 같은 극단값은 아직 눈으로 안 봤다.**
5. ~~`objc2-*` 크레이트의 **feature flag 이름**과 CF 타입 캐스팅 방식.~~
   **해소 (1·3단계).** 1단계에서 AppKit/CG 계열, 3단계에서 CoreText/CF 계열 확정.
   함정은 스파이크 결과 문서 "발견 3"에 정리.
6. ~~`CGColorSpace`를 **DeviceRGB로 할지 sRGB로 할지**~~
   **해소 (3단계). 양쪽 다 sRGB로 통일한다** — 비트맵 컨텍스트도 sRGB, `CGColor`도 `new_srgb`.
   색공간이 다르면 CoreGraphics가 변환을 끼워넣어 `#FF0000`이 버퍼에서 정확히 (0,0,255,a)로 안 떨어진다.
   `a_per_line_colour_overrides_the_global_one` 테스트가 이걸 고정한다.
7. ~~`CTRunGetGlyphsPtr`/`CTRunGetPositionsPtr`는 **NULL을 반환할 수 있다**~~
   **해소 (3단계). `_ptr` 변형을 아예 쓰지 않는다.** 항상 복사 버전(`CTRunGetGlyphs(range, buf)`)을
   쓴다 — 초당 한 줄당 memcpy 한 번은 무시할 만하고, NULL 분기를 잊어 조용히 아무것도 안 그리는
   함정이 사라진다.
8. ~~GH arm64 러너의 **Rosetta 2 사전 설치 여부**~~
   **해소 (8단계). 안 하기로 했다.** x86_64 슬라이스를 러너에서 테스트하지 않는다 — 그게 증명할 것
   (같은 Rust 코드가 통과한다)은 arm64 실행이 이미 증명한다. `cargo test`는 호스트에서만 돈다.
...
| 단계 | 결과 |
|---|---|
| `lipo` 직후 `codesign --verify` | `code object is not signed at all` — **lipo가 서명을 실제로 날린다** |
| `codesign --force --deep --sign -` | `flags=0x2(adhoc)`, `Signature=adhoc`, `TeamIdentifier=not set` |
| `codesign --verify --deep --strict` | `valid on disk` / `satisfies its Designated Requirement` |
| `spctl -a` | **`rejected`** — 공증이 없으니 Gatekeeper는 여전히 거부한다 (예상대로) |
| `ditto` 왕복 후 | 실행 권한(`-rwxr-xr-x`)과 서명 모두 보존 |
| 번들 실행 | `type="UIElement"`(LSUIElement 적용), `Version` 치환됨, `Arch=ARM64` |
