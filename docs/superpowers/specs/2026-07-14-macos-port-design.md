...
impl Panels {
    pub fn new() -> Result<Self>;
    /// Windows: WorkerW 생존 확인/재획득(backoff 포함). macOS: 항상 Ok(true).
    /// `false` = 아직 붙지 못했다(트레이에 경고 표시).
    pub fn ensure_attached(&mut self) -> Result<bool>;
...
"붙었는가/못 붙었는가"만 알면 되고, macOS는 그런 개념이 아예 없다.
