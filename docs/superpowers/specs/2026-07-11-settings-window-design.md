...
파일을 감시(파일 이벤트가 렌더러 메시지 루프를 깨우고 80ms 디바운스 후 재로드)하므로, 설정 창은
파일을 저장할 뿐이고 **IPC는 없다.** 값을 바꾸면 바탕화면의 카운트다운이 바로 바뀐다 — 이 앱의
핫 리로드 강점을 그대로 노출한다.
...
**자동 저장.** "적용"/"저장" 버튼이 없다. 위젯 값이 바뀌면 `config::save`로 `config.toml`에
쓰되, 쓰기 간격은 최소 100ms(`SAVE_INTERVAL_MS`)로 제한한다. 렌더러가 그 저장을 감지해 바탕화면에
반영한다. 실제 바탕화면이 라이브 미리보기 역할을 한다.

스로틀이 필요한 이유: 슬라이더를 드래그하면 프레임마다 값이 바뀌는데, 매 프레임 파일을 쓰면
디스크와 렌더러 감시가 폭주한다. 대신 트레일링 디바운스(마지막 변경 후 N ms 잠잠하면 쓰기)는
쓰지 않는다 — 조작 하나하나가 조용해질 때까지 기다려야 해서 바탕화면 반영이 늘 한 박자 늦고,
드래그 중에는 아예 반영되지 않는다. 선행 쓰기(첫 변경은 즉시) + 100ms 스로틀이면 클릭은 즉시,
드래그는 초당 10회 반영된다. 조작이 끝난 뒤 남은 마지막 값은 스로틀 만료 프레임에서 쓴다
(`ui`가 dirty일 때만 `request_repaint_after`로 그 프레임을 예약한다).
...
편집 루프(매 프레임): 위젯이 메모리 상태를 갱신 → 변경이 있으면 "dirty" → dirty이고 마지막
*쓰기* 후 100ms 경과했으면: 메모리 상태를 `Config`로 조립 → `validate` → 통과 시 `config::save`,
실패 시 경고 표시하고 저장 보류 → dirty 해제. 스로틀에 걸려 못 쓴 변경은 dirty로 남고, 그
프레임에서 `request_repaint_after`로 스로틀 만료 시점의 프레임을 예약해 거기서 쓴다.

종료: 창을 닫으면 마지막 dirty 상태를 한 번 flush(스로틀에 걸려 대기 중이던 변경을 잃지 않도록).
...
- 저장 스로틀 로직 — 순수 함수(`should_save(dirty, ms_since_last_write, min_interval_ms)`)로 뽑아
  단위 테스트.
