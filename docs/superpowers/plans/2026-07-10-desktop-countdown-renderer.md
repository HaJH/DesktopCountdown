...
**Architecture:** WorkerW(벽지 레이어)의 자식 창에 DirectComposition 비주얼을 붙이고, 그 표면에 DirectWrite + Direct2D로 글자를 그린다. 자식 창에서 픽셀별 알파를 합성하는 방법은 이것뿐이다 — `UpdateLayeredWindow`는 조용히 아무것도 그리지 않고, 평범한 자식 창은 Explorer의 벽지 재도색에 덮인다(스파이크로 확인). Win32를 아는 모듈과 모르는 모듈을 갈라 후자를 전부 단위 테스트한다. 숨은 최상위 컨트롤러 창이 타이머와 시스템 메시지를 받는다.

**Tech Stack:** Rust 2021 / `windows` 0.62 (Win32, Direct2D, DirectWrite, DirectComposition, D3D11, DXGI, WIC) / `windows-numerics` 0.3 / `jiff` 0.2 / `serde` + `toml` / `notify` 8.2 / `tray-icon` 0.24 / `tracing`
...
- 크레이트 버전: `windows = "0.62"`, `windows-numerics = "0.3"`, `jiff = "0.2"`, `serde = "1"`, `toml = "1"`, `notify = "8"`, `tray-icon = "0.24"`, `tracing = "0.1"`, `tracing-appender = "0.2"`, `thiserror = "2"`, `anyhow = "1"`.
- **`windows` 0.62에는 `implement` 피처가 없다.** `#[implement]` 매크로는 `windows::core::implement`로 무조건 노출된다.
- **`DrawTextLayout(origin)`, `D2D1_ELLIPSE::point`, `CreateTransformedGeometry(transform)`의 타입은 `windows_numerics::Vector2` / `Matrix3x2`다.** `D2D_POINT_2F`가 아니다.
- **`DefWindowProcW`는 Rust ABI 함수라 `WNDCLASSW::lpfnWndProc`에 직접 대입할 수 없다.** `extern "system"` 트램폴린을 쓴다.
...
### Task 1: 프로젝트 스캐폴드 + 스파이크  — 완료

커밋 `3920eb9`, `9b8ee30`, `64f08cc`. 결과는 `docs/superpowers/plans/spike-result.md`.

**스파이크는 실패했다.** `WS_EX_LAYERED` 자식 창 + `UpdateLayeredWindow`는 배경 레이어에서 아무것도
그리지 않는다 — 호출은 `Ok(())`를 반환한다. 같은 코드가 최상위 창에서는 정상 동작하므로 픽셀 버퍼는
문제가 아니었다. 그 결과 설계 §3.3/§8/§9/§12와 이 계획의 Task 9~12가 DirectComposition 방식으로 다시
쓰였다.

스파이크와 진단 바이너리는 역할을 다했으므로 삭제했다. 동작하는 DirectComposition 참고 코드는
커밋 `64f08cc`의 `src/bin/diag_dcomp.rs`에 남아 있다.

스캐폴드(`Cargo.toml`, `src/lib.rs`, `src/main.rs`)는 그대로 유효하다. **리포의 `Cargo.toml`이 이 문서의
예시보다 우선한다** — `implement` 피처는 존재하지 않고, DirectComposition·D3D11·DXGI 피처와
`windows-numerics`가 추가되었다.
...
설계 §3.2. **DirectComposition을 모르는** 그리기 모듈. 그릴 대상을 `&ID2D1RenderTarget`으로만 받는다.
`ID2D1DeviceContext`가 `ID2D1RenderTarget`을 상속하므로, 실행 시에는 DComp 표면의 컨텍스트를,
테스트에서는 WIC 비트맵 렌더 타깃을 넘긴다.
...
- Produces:
  ...
  - `pub struct Painter`
  - `pub fn Painter::new() -> anyhow::Result<Painter>`
  - `pub fn Painter::d2d_factory(&self) -> &ID2D1Factory1`
  - `pub fn Painter::measure(&self, lines: &Lines, style: &Style) -> anyhow::Result<(u32, u32)>`
  - `pub fn Painter::paint(&self, rt: &ID2D1RenderTarget, lines: &Lines, style: &Style, origin: (f32, f32)) -> anyhow::Result<()>`

`origin`은 `IDCompositionSurface::BeginDraw`가 돌려주는 오프셋이다. 표면이 아틀라스의 일부일 수 있어
모든 좌표에 더해야 한다. 테스트에서는 `(0.0, 0.0)`을 넘긴다.
...
mod tests {
    ...
    fn draw(p: &Painter, lines: &Lines, style: &Style) -> (Vec<u8>, u32, u32) {
        ...
        unsafe {
            ...
            let mut stride = 0u32;
            lock.GetStride(&mut stride).unwrap();
            let mut size = 0u32;
...
Expected: 컴파일 실패. `cannot find type 'Painter'`.
...
impl Painter {
    ...
    pub fn paint(
        ...
        unsafe {
            ...
            if style.shadow {
                self.pass(rt, &c, sum_x + SHADOW_OFFSET, sum_y + SHADOW_OFFSET,
                          main_x + SHADOW_OFFSET, main_y + SHADOW_OFFSET, style, &shadow)?;
            }
...
Run: `cargo test render`
Expected: 10개 PASS. 병렬 실행이어도 각 테스트가 자기 스레드에서 COM을 초기화하므로 플래그가 필요 없다.

`alpha_is_premultiplied`가 실패하면 `SetTextAntialiasMode`가 GRAYSCALE로 설정되지 않은 것이다.
`opacity_scales_alpha`의 허용 범위(120..=136)는 반올림 여유다. 크게 벗어나면 브러시 알파에 `opacity`가
곱해지지 않은 것이다.
...
- ID2D1RenderTarget만 받아 그린다. DirectComposition을 모른다.
- 테스트는 WIC 비트맵 렌더 타깃에 그려 픽셀을 검증
- 알파 합성을 위해 텍스트 AA를 GRAYSCALE로 고정
- 폰트 폴백, tabular figures, 자간, 오프셋 그림자, 불투명도"
```
...
**Interfaces:**
- Consumes: `ID2D1Factory1`, `IDWriteTextLayout` (Task 9)
- Produces:
  - `pub(crate) fn collect_geometry(d2d: &ID2D1Factory1, layout: &IDWriteTextLayout, origin_x: f32, origin_y: f32) -> anyhow::Result<Vec<ID2D1Geometry>>`
...
Run: `cargo test render`
Expected: `outline_mode_draws_less_ink_than_fill` FAIL — outline 모드가 아무것도 그리지 않아 커버리지가 0이다.
...
- [ ] **Step 4: Painter의 `pass`에 stroke 경로 연결**

`src/render/mod.rs`에 `mod outline;`을 추가한다. `pass`가 브러시를 둘 받도록 바꾸고, stroke 경로를 넣는다.
...
`paint`에서 브러시를 하나 더 만들고 호출부를 고친다:
...
그림자 패스는 두 브러시 모두 `&shadow`를 넘기고, 본 패스는 `&ink`, `&line`을 넘긴다.
...
`outline_mode_leaves_glyph_centres_transparent`가 실패하면 `DrawMode::Outline`에서도 `DrawTextLayout`이
호출되고 있거나, `DrawGeometry` 대신 `FillGeometry`를 부르고 있는 것이다.
...
### Task 11: WorkerW 확보와 자식 창

설계 §3.3. **스파이크로 검증된 절차를 그대로 옮긴다.** 근거는 `docs/superpowers/plans/spike-result.md`,
동작하는 참고 코드는 커밋 `64f08cc`의 `src/bin/diag_dcomp.rs`.

**Files:**
- Create: `src/workerw.rs`
- Modify: `src/lib.rs`

**Interfaces:**
- Consumes: `crate::layout::Rect` (Task 6)
- Produces:
  - `pub fn acquire() -> anyhow::Result<HWND>`
  - `pub fn is_alive(hwnd: HWND) -> bool`
  - `pub struct ChildWindow`
  - `pub fn ChildWindow::create(parent: HWND) -> anyhow::Result<ChildWindow>`
  - `pub fn ChildWindow::hwnd(&self) -> HWND`
  - `pub fn ChildWindow::place(&self, parent: HWND, rect: Rect) -> anyhow::Result<()>`
  - `pub fn ChildWindow::raise_if_covered(&self, parent: HWND)`
  - `impl Drop for ChildWindow` — `DestroyWindow`

- [ ] **Step 1: 테스트 작성**

`src/workerw.rs` 하단에. 살아 있는 Explorer 데스크톱이 필요한 것은 `#[ignore]`로 둔다.
...
`src/lib.rs`에 `pub mod workerw;` 추가 후,

Run: `cargo test workerw`
Expected: 컴파일 실패. `cannot find function 'is_alive'`.
...
unsafe fn lookup(progman: HWND) -> Option<HWND> {
    if let Some(h) = classic_sibling() {
        return Some(h);
    }
    FindWindowExW(Some(progman), None, w!("WorkerW"), None).ok()
}
...
Run: `cargo test workerw`
Expected: `a_null_handle_is_not_alive` PASS, 2개 ignored.

Run: `cargo test workerw -- --ignored`
Expected: 2개 PASS. `creates_a_child_and_places_it_in_screen_coordinates`가 실패하면 `ScreenToClient`
변환을 빠뜨린 것이다 — 자식 창의 `SetWindowPos` 좌표는 부모 클라이언트 기준이다.

- [ ] **Step 5: 커밋**

```bash
git add src/workerw.rs src/lib.rs
git commit -m "workerw: 벽지 레이어 창 확보와 자식 창 부착

- 고전(최상위 형제)과 Progman 자식 배치를 모두 지원
- CreateWindowEx가 타 프로세스 부모를 거부하므로 최상위 생성 후 SetParent
- 스크린 좌표를 부모 클라이언트 좌표로 변환해 배치
- 다른 벽지 앱에 덮이면 최상단으로 올림"
```

### Task 12: DirectComposition 표면

설계 §3.3. 자식 창에 픽셀별 알파를 공급하는 유일한 방법. 동작하는 참고 코드는 커밋 `64f08cc`의
`src/bin/diag_dcomp.rs`.
...
**Interfaces:**
- Consumes: `crate::render::Painter::d2d_factory` (Task 9)
- Produces:
  - `pub struct Compositor`
  - `pub fn Compositor::new(d2d_factory: &ID2D1Factory1) -> anyhow::Result<Compositor>`
  - `pub struct Surface` + `pub fn Surface::size(&self) -> (u32, u32)`
  - `pub fn Compositor::attach(&self, hwnd: HWND) -> anyhow::Result<Surface>`
  - `pub fn Compositor::draw<F>(&self, s: &mut Surface, w: u32, h: u32, f: F) -> anyhow::Result<()>`
    where `F: FnOnce(&ID2D1RenderTarget, (f32, f32)) -> anyhow::Result<()>`

`draw`가 콜백에 넘기는 `(f32, f32)`는 `IDCompositionSurface::BeginDraw`의 오프셋이다. 그대로
`Painter::paint`의 `origin`에 넣는다.

- [ ] **Step 1: 스모크 테스트 작성**

`src/dcomp.rs` 하단에. WorkerW는 필요 없다 — 숨은 최상위 창에 붙여도 API 경로는 동일하다.
...
`src/lib.rs`에 `pub mod dcomp;` 추가 후,

Run: `cargo test dcomp`
Expected: 컴파일 실패. `cannot find type 'Compositor'`.
...
pub struct Compositor {
    ...
    /// Held so the DXGI device outlives everything derived from it.
    _d3d: ID3D11Device,
}
...
impl Compositor {
    pub fn new(d2d_factory: &ID2D1Factory1) -> Result<Self> {
        unsafe {
            ...
            let d3d = d3d.expect("D3D11CreateDevice succeeded without a device");
...
`creates_a_device`가 `DXGI_ERROR_UNSUPPORTED`로 실패하면 `D3D_DRIVER_TYPE_HARDWARE` 대신
`D3D_DRIVER_TYPE_WARP`로 폴백하는 코드를 넣는다. 원격 데스크톱 세션에서 실제로 일어난다.

- [ ] **Step 5: 커밋**

```bash
git add src/dcomp.rs src/lib.rs
git commit -m "dcomp: DirectComposition 표면

- D3D11 + DXGI + DComp 디바이스, 창마다 타깃/비주얼/표면
- 크기가 바뀌면 표면을 다시 만든다
- BeginDraw의 오프셋을 그리기 콜백에 전달
- 콜백이 실패해도 EndDraw를 호출해 표면 잠금을 푼다"
```

### Task 13: 앱 오케스트레이션 — 컨트롤러 창과 틱 루프
...
- 문자열이 같으면 렌더 스킵
- 모니터마다 자식 창 + DirectComposition 표면
- WorkerW 소실 감지 후 재부착, WM_DISPLAYCHANGE 시 창 재생성"
```

---

### Task 14: 설정 파일 감시와 핫 리로드
...
### Task 16: 복원력 — WorkerW 백오프와 컴포지터 재생성
...
**(1) 시작 시 WorkerW를 못 찾아도 죽지 않는다.** `App::run`의 `workerw: workerw::acquire()?`를
`workerw: HWND(std::ptr::null_mut())`로 바꾸고, `rebuild_panels()` 호출을 지운다. 첫 `tick()`이
...
**(3) 그리기 실패 시 컴포지터를 한 번 재생성한다.** GPU 드라이버가 갱신되거나 원격 데스크톱으로
전환되면 D3D11 디바이스가 소실되고 `DXGI_ERROR_DEVICE_REMOVED`가 난다. 그때는 D3D11부터 다시 만들어야
하므로, `Compositor`를 통째로 새로 만들고 패널의 표면을 다시 붙인다. `Painter`는 D2D 팩토리와 DirectWrite
팩토리뿐이라 소실되지 않는다.

`tick()`의 `self.draw(&lines)?` 호출을 아래로 바꾼다.
...
`rebuild_panels()`가 `self.last_lines`를 `None`으로 되돌리므로, 재시도 후의 `self.last_lines = Some(lines)`가
다음 틱의 스킵 판정을 올바르게 만든다.

`tick()`이 `Err`를 리턴하면 `wndproc`이 로그만 남기고 다음 틱을 계속 돌린다. 앱은 죽지 않는다.
...
git commit -m "복원력: WorkerW 지수 백오프와 컴포지터 재생성

- 시작 시 WorkerW가 없어도 앱이 죽지 않고 재시도
- 재시도 예산(60초) 소진 시 트레이만 남겨 종료 가능하게 유지
- 그리기 실패 시 D3D11부터 컴포지터를 재생성하고 패널을 다시 붙인 뒤 재시도"
```
...
- 카운트다운이 바탕화면 아이콘 아래, Wallpaper Engine 배경 위에 그려지고 바탕화면 새로 고침에도 살아남는다.
